 ...
 *
 * Extrahiert aus dem Landing-Editor, damit Landing-Pages UND Journal-Beiträge
 * denselben Block-Builder nutzen (Palette · Vorschau mit Reorder · Eigenschaften
 * mit RU/EN/DE-Tabs, Bild-Upload, bg-Picker).
 *
 *   ┌──────────┬───────────────────────┬───────────────┐
 *   │ Palette  │   Vorschau (Reorder)  │ Eigenschaften │
 *   └──────────┴───────────────────────┴───────────────┘
 *
 * Stateless gegenüber dem Block-Array: hält selektierten Index/Drag-Status
 * intern, gibt Änderungen am Array via onChange nach oben. Bild-Upload und ob
 * der product_grid-Block erlaubt ist, kommen als Props (Landing erlaubt ihn,
 * Journal nicht).
 * ────────────────────────────────────────────────────────────────────────── */

// Bild-Upload-Action: gibt bei Erfolg eine URL zurück. Bewusst die breitere
// Shape der bestehenden Upload-Actions (storyBildUploadAction), damit Landing-
// und Journal-Editor dieselbe Action ohne Wrapper-Friktion durchreichen können.
export type BlockUploadAction = (
  fd: FormData,
) => Promise<{ ok: boolean; url?: string; error?: string }>;

const PALETTE_ALL: { type: LandingBlockType; label: string; icon: React.ElementType; sub: string }[] = [
...
const labelFor = (t: LandingBlockType) => PALETTE_ALL.find((p) => p.type === t)?.label ?? t;

export function BlockComposer({
  blocks,
  onChange,
  uploadAction,
  produktQuelleErlaubt = true,
}: {
  blocks: LandingBlock[];
  onChange: (b: LandingBlock[]) => void;
  uploadAction: BlockUploadAction;
  produktQuelleErlaubt?: boolean;
}) {
  const PALETTE = produktQuelleErlaubt
    ? PALETTE_ALL
    : PALETTE_ALL.filter((p) => p.type !== "product_grid");
  ...
  // ── Block-Manipulation (geben jeweils das neue Array via onChange weiter) ──
  const add = (t: LandingBlockType) => {
    const next = [...blocks, { ...NEU[t] }];
    onChange(next);
    setSel(next.length - 1);
  };
  const remove = (i: number) => {
    const n = blocks.filter((_, idx) => idx !== i);
    onChange(n);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr_320px] border border-vintage-sand" style={{ borderRadius: "var(--radius-card)", overflow: "hidden", minHeight: 480 }}>
      ...
      <div className="border-l border-vintage-sand bg-vintage-white p-4 overflow-y-auto">
        ...
        {sel === null || !blocks[sel] ? (
          ...
          <BlockProps block={blocks[sel]} onPatch={(p) => patch(sel, p)} uploadAction={uploadAction} />
        )}
      </div>
    </div>
  );
