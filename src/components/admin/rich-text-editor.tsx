...
export function RichTextEditor({
  ...
  return (
    <LexicalComposer
      initialConfig={{
        ...
        theme: {
          ...
          quote: "border-l-2 border-vintage-gold pl-3 italic text-vintage-brown my-2",
          link: "text-vintage-gold underline",
    ...
    >
      <div className="border border-vintage-sand bg-vintage-white" style={{ borderRadius: "var(--radius-card)" }}>
        <Toolbar />
        <div className="relative px-3 py-2 text-sm text-vintage-espresso min-h-[180px]">
          <RichTextPlugin
            contentEditable={<ContentEditable className="outline-none min-h-[160px]" />}
            placeholder={<div className="absolute top-2 left-3 text-vintage-dust pointer-events-none">{placeholder}</div>}
...
function Toolbar() {
  ...
  const Btn = ({ onClick, title, children }: { onClick: () => void; title: string; children: React.ReactNode }) => (
    <button type="button" onMouseDown={e => e.preventDefault()} onClick={onClick} title={title}
      className="p-1.5 text-vintage-brown hover:bg-vintage-parchment rounded transition-colors">
      {children}
    </button>
  );

  return (
    <div className="flex items-center gap-0.5 flex-wrap px-2 py-1.5 border-b border-vintage-sand">
      ...
      <span className="w-px h-4 bg-vintage-sand mx-1" />
