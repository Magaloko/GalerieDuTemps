...
const RegistrierungsSchema = z.object({
  ...
  passwort:          z.string().min(8, "Минимум 8 символов"),
  ...
  agb_akzeptiert:    z.literal("on", { message: "Примите Партнёрское соглашение" }),
  datenschutz_akzeptiert: z.literal("on", { message: "Примите Политику конфиденциальности" }),
}).refine(d => d.passwort === d.passwort_wdh, {
  message: "Пароли не совпадают",
  path:    ["passwort_wdh"],
}).refine(d => d.ist_kleinunternehmer || d.gewerbe_angemeldet, {
  message: "Вы должны быть зарегистрированы как ИП или ТОО",
...
export async function registrierenAction(
  ...
  if (!settings.registrierung_offen) {
    return { fehler: "Регистрация сейчас закрыта." };
  }
  ...
  if (existing) return { fehler: "Этот e-mail уже зарегистрирован. Войдите или восстановите пароль." };
  ...
  if (data.sponsor_code) {
    ...
    if (!sponsor) {
      return { errors: { sponsor_code: ["Реферальный код недействителен или аккаунт не активен"] } };
    }
  ...
  sendEmail({
    ...
    subject: "Добро пожаловать в партнёрскую программу Galerie du Temps",
  ...
  return `
    ...
    <body style="font-family: Georgia, serif; background: #F5F0E8; margin: 0; padding: 40px 20px;">
      <div style="max-width: 520px; margin: 0 auto; background: #FDFAF5; border: 1px solid #C9B89A; padding: 48px;">
        ...
        <p style="color: #4A2C1A;">Здравствуйте, ${vorname}!</p>
        <p style="color: #4A2C1A; line-height: 1.7;">
          Спасибо за регистрацию в партнёрской программе! Ваш аккаунт скоро будет
          проверен и активирован. Как только это произойдёт, вы получите ещё одно
          письмо и сможете начать работу.
        </p>
        <div style="background: #E8DFD0; border-left: 3px solid #C9A84C; padding: 16px 20px; margin: 24px 0;">
          <p style="margin: 0; color: #8B6F47; font-size: 12px; text-transform: uppercase; letter-spacing: 2px;">Ваш реферальный код</p>
          <p style="margin: 8px 0 0; color: #4A2C1A; font-family: monospace; font-size: 24px; letter-spacing: 4px;">${referralCode}</p>
        </div>
        <p style="color: #8B6F47; line-height: 1.7;">С уважением,<br><strong style="color: #4A2C1A;">команда Galerie du Temps</strong></p>
      </div>
    </body>
    </html>
  `.trim();
