  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <h1 className="font-serif text-3xl text-vintage-cream">Выплаты</h1>
        <p className="text-vintage-dust text-sm font-sans mt-1">История и следующая выплата</p>
      </div>

      {/* Nächste Auszahlung */}
      <div className="bg-vintage-espresso text-vintage-cream p-6" style={{ borderRadius: "var(--radius-card)" }}>
        <p className="text-vintage-gold text-xs tracking-widest uppercase mb-2">Готово к выплате</p>
        <p className="font-serif text-4xl">{formatPreis(bestaetigt / 100)}</p>
        <p className="text-vintage-cream/60 text-xs mt-2 font-sans">
          {erreicht
            ? `Минимальная сумма достигнута (${formatPreis(mindest / 100)}). Выплата будет произведена в ближайшем цикле.`
            : `Минимальная сумма: ${formatPreis(mindest / 100)} — осталось ${formatPreis((mindest - bestaetigt) / 100)} до следующей выплаты.`
          }
        </p>
      </div>
      ...
      <section className="bg-vintage-brown border border-vintage-sand/40 p-6" style={{ borderRadius: "var(--radius-card)" }}>
        <h2 className="font-serif text-lg text-vintage-cream mb-5">История выплат</h2>
        {auszahlungen.length === 0 ? (
          <div className="text-center py-10">
            <Wallet className="w-10 h-10 text-vintage-sand mx-auto mb-3" />
            <p className="text-vintage-dust text-sm font-sans">Выплат пока нет</p>
          </div>
        ) : (
              ...
              <div key={a.id} className="py-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-vintage-dust font-sans flex items-center gap-1">
                    {a.status === "bezahlt"
                      ? <><CheckCircle2 className="w-3 h-3 text-vintage-sage" /> Выплачено {a.bezahlt_am ? new Date(a.bezahlt_am).toLocaleDateString("ru-RU") : ""}</>
                      : <><Clock className="w-3 h-3 text-vintage-gold" /> Создано {new Date(a.erstellt_am).toLocaleDateString("ru-RU")}</>
                    }
                  </p>
                </div>
