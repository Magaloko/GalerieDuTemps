    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex p-4 bg-vintage-sage/10 border border-vintage-sage/30 mb-6" style={{ borderRadius: "50%" }}>
          <CheckCircle2 className="w-10 h-10 text-vintage-gold" />
        </div>
        ...
        <p className="text-vintage-dust font-sans">
          {t.cart.bestaetigung_text} <strong className="text-vintage-cream/80">{order.customer_email}</strong>
        </p>
      </div>

      <div className="bg-vintage-brown border border-vintage-sand/40 p-6 space-y-5" style={{ borderRadius: "var(--radius-card)" }}>
        {/* Bestellnummer */}
        <div className="flex items-center justify-between pb-4 border-b border-vintage-sand/40">
          <div>
            <p className="text-xs font-sans uppercase tracking-widest text-vintage-dust">{t.cart.bestellnummer}</p>
            <p className="font-mono text-xl text-vintage-cream mt-1">GDT-{order.order_number}</p>
          </div>
          <Package className="w-8 h-8 text-vintage-gold" />
        </div>

        {/* Items */}
        <div className="divide-y divide-vintage-sand/20">
          {(order.items ?? []).map(item => (
            <div key={item.id} className="flex items-center gap-3 py-3">
              <div className="w-12 h-12 bg-vintage-brown/40 overflow-hidden flex-shrink-0" style={{ borderRadius: "var(--radius-vintage)" }}>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-serif text-vintage-cream truncate">{item.produkt_name}</p>
        ...
        <div className="space-y-1 text-sm font-sans border-t border-vintage-sand/40 pt-4">
          ...
          <div className="flex justify-between font-serif text-vintage-cream text-lg pt-2 border-t border-vintage-sand/40">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8">
        ...
        {order.customer_id && (
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 px-6 py-3 border border-vintage-sand/40 text-vintage-cream/80 font-sans text-xs tracking-widest uppercase hover:bg-vintage-brown/40 transition-colors"
            style={{ borderRadius: "var(--radius-button)" }}
          >
            {t.cart.zum_konto}
          </Link>
