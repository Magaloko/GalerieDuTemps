...
export const metadata: Metadata = { title: "Newsletter bestätigt" };
...
export default async function BestaetigtPage({
  ...
  // Welcome-Coupon-Mail (Best-Effort, falls Sub bestätigt)
  if (sub) {
    sendEmail({
      to:      [{ email: sub.email, name: sub.vorname ?? "" }],
      subject: "Willkommen bei Galerie du Temps – dein Coupon",
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-20">
        <div className="max-w-md text-center">
          ...
          <h1 className="font-serif text-3xl text-vintage-espresso mb-4">
            {sub ? "Newsletter bestätigt!" : "Link ungültig"}
          </h1>
          {sub ? (
            <>
              <p className="text-vintage-dust font-sans text-sm mb-3">
                Vielen Dank! Du erhältst gleich eine Willkommens-Mail mit deinem Coupon.
              </p>
              <div className="bg-vintage-gold/10 border border-vintage-gold/30 p-4 my-6" style={{ borderRadius: "var(--radius-card)" }}>
                <p className="text-xs font-sans uppercase tracking-widest text-vintage-dust mb-1 flex items-center justify-center gap-1">
                  <Sparkles className="w-3 h-3 text-vintage-gold" /> Dein Code
                </p>
                <p className="font-mono text-xl tracking-widest text-vintage-gold">WELCOME10</p>
                <p className="text-xs text-vintage-dust font-sans mt-1">10% auf deine erste Bestellung</p>
              </div>
            </>
          ) : (
            <p className="text-vintage-dust font-sans text-sm mb-6">
              Der Bestätigungs-Link ist ungültig oder abgelaufen. Bitte melde dich erneut an.
            </p>
          )}
          <Link href={sub ? "/katalog" : "/newsletter/anmelden"}
            ...
            {sub ? "Zum Katalog" : "Erneut anmelden"}
          </Link>
        </div>
      </main>
      <SiteFooter />
    ...
    <!DOCTYPE html><html lang="de"><body style="font-family: Georgia, serif; background: #F5F0E8; margin: 0; padding: 40px 20px;">
      <div style="max-width: 520px; margin: 0 auto; background: #FDFAF5; border: 1px solid #C9B89A; padding: 48px;">
        ...
        <p style="color: #4A2C1A; line-height: 1.7;">
          danke für deine Anmeldung! Als kleines Willkommens-Geschenk bekommst du
          10% Rabatt auf deine erste Bestellung mit folgendem Code:
        </p>
