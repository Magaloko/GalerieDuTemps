  ...
  return (
    <tr className={`hover:bg-vintage-parchment/30 transition-colors ${pending ? "opacity-50" : ""}`}>
      ...
      <td className="px-4 py-3 text-center">
        {aktiv ? (
          <span className="px-2 py-0.5 text-xs bg-vintage-sage/10 text-vintage-sage" style={{ borderRadius: "var(--radius-vintage)" }}>aktiv</span>
        ) : subscriber.unsubscribed_am ? (
          <span className="px-2 py-0.5 text-xs bg-vintage-burgundy/10 text-vintage-burgundy" style={{ borderRadius: "var(--radius-vintage)" }}>abgemeldet</span>
        ) : (
          <span className="px-2 py-0.5 text-xs bg-vintage-gold/10 text-vintage-gold" style={{ borderRadius: "var(--radius-vintage)" }}>unbestätigt</span>
        )}
      </td>
      <td className="px-4 py-3 text-vintage-dust text-xs">{new Date(subscriber.erstellt_am).toLocaleDateString("de-DE")}</td>
      <td className="px-4 py-3 text-right">
        <button onClick={() => { if (confirm("Wirklich löschen?")) startTransition(() => subscriberDeleteAction(subscriber.id)); }}
          disabled={pending}
