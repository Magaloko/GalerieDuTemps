    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <h1 className="font-serif text-2xl text-vintage-espresso">Rechnungen</h1>
          <p className="text-vintage-dust text-xs font-sans mt-0.5">{daten.gesamt} Rechnungen</p>
        </div>
      </div>
      ...
      {daten.items.length === 0 ? (
        <div className="text-center py-16 bg-vintage-white border border-vintage-sand" style={{ borderRadius: "var(--radius-card)" }}>
          ...
          <p className="font-serif text-vintage-brown">Keine Rechnungen</p>
          <p className="text-xs text-vintage-dust font-sans mt-1">Werden erstellt sobald Bestellungen bezahlt sind.</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm font-sans">
              <thead className="bg-vintage-parchment/50 border-b border-vintage-sand">
                <tr>
                  <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-vintage-dust font-normal">Beleg-Nr.</th>
                  <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-vintage-dust font-normal">Datum</th>
                  <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-vintage-dust font-normal">Empfänger</th>
                  <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-vintage-dust font-normal">Bestellung</th>
              ...
              <tbody className="divide-y divide-vintage-sand/40">
                {daten.items.map(r => (
                  <tr key={r.id} className="hover:bg-vintage-parchment/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-vintage-dust">{new Date(r.rechnungs_datum).toLocaleDateString("de-DE")}</td>
