...
export const metadata: Metadata = { title: "Kunden-Detail" };
export const dynamic = "force-dynamic";

const TYPE_LABEL: Record<string, string> = {
  b2c: "Privatkund:in",
  b2b_pending: "B2B – wartet auf Freischaltung",
  b2b_verified: "B2B – verifiziert ✓",
  b2b_rejected: "B2B – abgelehnt",
};
    ...
    <div className="space-y-6 max-w-6xl">
      <nav className="flex items-center gap-2 text-xs font-sans text-vintage-dust">
        <Link href="/admin/kunden" className="hover:text-vintage-brown flex items-center gap-1 transition-colors">
          <ChevronLeft className="w-3 h-3" /> Kund:innen
        </Link>
        <span>/</span>
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-4 mt-3 text-sm font-sans text-vintage-dust">
              ...
              <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> Seit {new Date(customer.erstellt_am).toLocaleDateString("de-DE")}</span>
            ...
            {customer.dnc_aktiv && (
              <p className="text-xs text-vintage-burgundy font-sans mt-2 italic">
                ⚠️ DNC aktiv seit {customer.dnc_seit && new Date(customer.dnc_seit).toLocaleDateString("de-DE")}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatBox icon={ShoppingBag} label="Bestellungen" wert={stats.bestellungen_anzahl} />
        <StatBox icon={Coins}       label="Gesamtwert"   wert={formatPreis(stats.bestellungen_summe_cent / 100)} />
        <StatBox icon={Calendar}    label="Letzte Bestellung" wert={stats.letzte_bestellung_am ? new Date(stats.letzte_bestellung_am).toLocaleDateString("de-DE") : "–"} />
        <StatBox icon={Calendar}    label="Offene Tasks" wert={stats.tasks_offen} />
      </div>

      {/* Pipeline-Stage */}
      <section className="bg-vintage-white border border-vintage-sand p-5" style={{ borderRadius: "var(--radius-card)" }}>
        <p className="text-xs font-sans uppercase tracking-widest text-vintage-dust mb-3">Pipeline-Stage</p>
      ...
      {/* Bestellungen */}
      <section className="bg-vintage-white border border-vintage-sand p-5" style={{ borderRadius: "var(--radius-card)" }}>
        <h2 className="font-serif text-lg text-vintage-espresso mb-3">Bestellungen ({orders.length})</h2>
        {orders.length === 0 ? (
          <p className="text-vintage-dust text-sm font-sans text-center py-4">Noch keine</p>
        ) : (
            ...
            {orders.slice(0, 10).map(o => (
              <Link key={o.id} href={`/admin/bestellungen/${o.id}`} className="py-2 flex items-center justify-between hover:bg-vintage-parchment/40 -mx-2 px-2 transition-colors">
                <div>
                  ...
                  <p className="text-xs text-vintage-dust">{new Date(o.erstellt_am).toLocaleDateString("de-DE")} · {o.status}</p>
                </div>
