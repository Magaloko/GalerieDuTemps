  ...
  return (
    <section className="bg-vintage-white border border-vintage-sand p-5" style={{ borderRadius: "var(--radius-card)" }}>
      <h2 className="font-serif text-lg text-vintage-espresso flex items-center gap-2 mb-4">
        <StickyNote className="w-4 h-4 text-vintage-gold" /> Notizen ({notes.length})
      </h2>
      ...
      {/* Liste */}
      {notes.length === 0 ? (
        <p className="text-xs text-vintage-dust font-sans text-center py-4 italic">Noch keine Notizen</p>
      ) : (
            ...
            <div key={n.id}
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  ...
                  <button onClick={() => {
                    if (confirm("Löschen?")) startTransition(() => noteDeleteAction(n.id, customerId));
              ...
              <p className="text-[10px] text-vintage-dust font-sans mt-1">
                {n.erstellt_von_name ?? "System"} · {new Date(n.erstellt_am).toLocaleString("de-DE")}
              </p>
            </div>
          ))}
        </div>
